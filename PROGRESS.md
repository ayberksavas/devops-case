...
39. **Host-level `socat` forwarder packaged as a systemd unit** (`minikube-ingress-proxy.service`) to bridge `EC2:80` to the minikube ingress at `192.168.49.2:80`. Discovered after the main flow was complete: with the docker driver, minikube doesn't bind to the host's port 80, so the Elastic IP wasn't actually serving traffic externally even though the cluster was healthy. Alternatives: (a) switch minikube to the `none` driver (binds directly to host, but invasive to a working cluster), (b) `kubectl port-forward --address 0.0.0.0` (dies on SSH disconnect, not robust), (c) iptables NAT rules (works but harder to reason about). Chose socat in systemd: tiny, well-understood, `Restart=always` for crash recovery, hardcoded IP `192.168.49.2` (docker driver's stable default — avoids the user/HOME mismatch that breaks running `minikube ip` as root). On a fresh EC2 boot the only manual step is `minikube start`; once that's up, the forwarder's retry loop picks up within ~10 seconds. Documented in `SETUP.md` (Phase 4) and `RUNBOOK.md` (Common failure modes).
40. **`values-prod.yaml` Ingress carries two rules — a named host (`devops-case.local`) AND a catch-all** (no `host` field, k8s spec treats this as matching any Host header). Reason: visiting `http://<EIP>/ping` in a browser sends `Host: <EIP>`, which doesn't match the named-host rule, so nginx-ingress returns 404 even though the cluster is healthy. The catch-all makes the public demo URL work for a raw browser click, not just `curl -H "Host: devops-case.local"`. The dev overlay keeps just the named host — dev access is via `/etc/hosts` + the documented hostname, no public IP concern. Template change in `charts/app/templates/ingress.yaml` conditionally omits the `host:` line when the value is empty (k8s requires the field to be absent, not present-and-empty, for catch-all behaviour).
